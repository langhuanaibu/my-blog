...
export function dailyCard(item, date, options = {}) {
  return `<article class="card report-card" data-item-id="${escapeHtml(item.id)}">
    <div class="card-top"><span class="tag cat-${escapeHtml(item.category)}">${escapeHtml(CATEGORY_LABELS[item.category] || item.category)}</span>${item.status ? `<span class="tag">${escapeHtml(item.status)}</span>` : ""}</div>
...
export function renderDailyReport(data, options = {}) {
  ...
  const sections = CATEGORY_KEYS.map((category) => {
    ...
    return `<section class="report-section" data-category="${category}" aria-labelledby="cat-${category}"><h2 id="cat-${category}" class="sec-title">${CATEGORY_LABELS[category]} <span class="n">${rows.length} 篇</span></h2><div class="grid">${rows.length ? rows.map((item) => dailyCard(item, data.date, options)).join("") : '<p class="section-empty">今日暂无精选</p>'}</div></section>`;
  }).join("");
...
export function renderDetail(item, type = "news", date = "", options = {}) {
  ...
  return `<article class="detail-wrap"><a class="dback" href="${routeUrl({ view: "reports", period: "day", date })}" data-route>← 返回 ${escapeHtml(date)} 当日</a><h1 class="detail-title">${escapeHtml(title)}</h1>${common}${body}<div class="srcs">${sourceLinks(item)}</div>${actionButtons(item, { ...options, date, type })}</article>`;
}
...
export function renderWeeklyReport(data) {
  ...
  return `<article class="weekly-report"><header class="brief"><div class="bt">${escapeHtml(data.week || "周报")}</div><h1>${escapeHtml(lead.title || "本周综述")}</h1>${lead.summary ? `<p>${escapeHtml(lead.summary)}</p>` : ""}${coverage.daily_count != null ? `<p class="coverage">覆盖 ${coverage.daily_count}/${coverage.expected_days || 7} 期${missing.length ? ` · 缺失：${missing.map(escapeHtml).join("、")}` : ""}</p>` : ""}</header>${Object.keys(stats).length ? `<dl class="weekly-stats"><div><dt>精选</dt><dd>${escapeHtml(stats.pick_count ?? 0)}</dd></div><div><dt>独立事件</dt><dd>${escapeHtml(stats.event_count ?? stats.unique_event_count ?? 0)}</dd></div><div><dt>信源</dt><dd>${escapeHtml(stats.source_count ?? 0)}</dd></div><div><dt>阅读</dt><dd>${escapeHtml(stats.read_minutes ?? 0)} 分钟</dd></div></dl>` : ""}<section><h2 class="sec-title">动态主题</h2>${(data.threads || []).map((thread) => `<article class="wk-thread"><h3>${escapeHtml(thread.title)} ${thread.direction ? `<span class="tag dir-${escapeHtml(thread.direction)}">${escapeHtml(thread.direction)}</span>` : ""}</h3><p>${escapeHtml(thread.summary || thread.one_liner || thread.detail || "")}</p><div class="representatives">${(thread.representative_refs || []).map((ref) => refLink(ref, "代表报道")).join("")}</div></article>`).join("") || '<p class="section-empty">暂无主题</p>'}</section>${(data.watch_recap || []).length ? `<section><h2 class="sec-title">上周判断回收</h2>${data.watch_recap.map((row) => `<article class="wk-recap"><strong>${escapeHtml(row.title || row.watch)}</strong><span>${escapeHtml(row.note || row.result || "")}</span>${(row.evidence_refs || []).map((ref) => refLink(ref, "支撑报道")).join("")}</article>`).join("")}</section>` : ""}${reading.length ? `<section><h2 class="sec-title">本周值得读</h2><div class="reading-list">${reading.map((row) => refLink(row.ref || row.item_ref, row.title || row.ref)).join("")}</div></section>` : ""}${(data.outlook || []).length ? `<section><h2 class="sec-title">下周信号</h2><ul>${data.outlook.map((row) => `<li>${escapeHtml(typeof row === "string" ? row : row.text || row.title)}</li>`).join("")}</ul></section>` : ""}</article>`;
}
