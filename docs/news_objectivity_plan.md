...
- 手动 `Objectivity Acceptance` workflow 复用 `LLM_API_KEY` 连跑三轮固定夹具，只上传
  聚合报告，不提交数据。2026-07-22 的 Run #29894388908 未通过：最差轮结构合法率
  22.22%、归因准确率 29.03%、总红线 2；三轮 Judge 自报红线均为 0、标签错配均为 2，
  但结构无效分别为 30/35/30 条。当前阻塞是 Judge 批次结构稳定性，`active` 继续禁用；
  后续实时状态与每日计数以 GitHub Issue #15 为准。
