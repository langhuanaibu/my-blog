...
- 2026-07-25 诊断轮 Run #30157820536 把剩余失败**全部定位到测量层**：三轮残留违规均为 0
  ...
  缺席型风险夹具上。据此完成上文的四门拆分、接受集与 prompt 修正。
- **45-case / three-run 夹具门已于 2026-07-25 通过**：Run #30159234858 `accepted: true`，
  最差轮残留红线 0、标签一致性 93.33%、归因 100%、结构 100%。三轮 `invalid_case_count`
  全为 0——上一轮的 8 次结构无效被 prompt 的缺席型风险说明一次性消除，证实那不是模型
  抖动；三轮归因均 31/31，证实此前第 2 轮的 90.32% 纯属结构失败污染分母。
  **通过夹具门只授予 active 的人工评审资格**：7-day shadow 指标门与人工评审仍在前面，
  `objectivity.mode` 保持 `interim`，`active` 继续禁用。实时状态与每日计数以
  GitHub Issue #15 为准。
...
- **已考虑并明确拒绝扩集的条目**（防止后来人随手加或重新讨论一遍）：`conflict-04`
  （"An aid group confirmed its clinic closed after nearby explosions but did not
  identify who launched them"）在 Run #30159234858 三轮全部被判 `sensitive_single_source`，
  形态上与已扩集的 `conflict-01/02/03` 属同一接缝，论证本身站得住。但当轮门已以
  93.33% 对 90% 的余量通过，**此时扩集的唯一动机就是拉高余量，正属上条纪律禁止的
  事后扩集，因此不做**。将来若某轮真因它失败，届时再按论证决定并把论证留档。
