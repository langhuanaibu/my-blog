...
详情深化后的 enrich 同时停用新闻 `significance` 与 `why`，全文模式增加详情走向及更深的来龙／起因与现状；公开 interim 对摘要材料安全短写。按证据分为丰富材料、有限全文、摘要材料，最终审计后记录各档数量、`detail` 字数中位数及丰富材料达到 300 字且至少两段的比例。自动化回归只证明输入和合同正确，仍需 5 个有效产出日检查实际文字质量。

- 每天从每个有内容的类目抽 1 条，五天最多 25 条。
- 丰富材料条目至少 80% 的最终 `detail` 达到 300 字且至少两段，丰富材料正文中位数目标 350–600 字。人工抽样至少 80% 同时通过：事实增量、无重复填充、无跨字段复述、归因正确和外行可读。起因片段还须逐条回溯且完整因果语义成立；短走向与详情走向的关键变量、可观察路标语义一致。
- 零增量成本门使用相同模型、价格和固定 45 条夹具做改动前后三轮配对；手动 workflow 从当前提交的父提交抽取旧管线作基线，只计 enrich 与内容审计／修复，不计独立 judge。调用数不得增加；费用按所有输入均为 cache miss 的标准化口径比较，实际缓存折扣费用只作诊断，避免基线先跑造成候选假性便宜。价格未知、签名不一致、截断、余额不足或运行不完整均不通过。该功能提交后应立即触发，后续无关提交的父版本不能替代本次基线证据。另取 5 个有效 shadow 日按每条入审精选的标准化费用复核，高于改动前同模式基线即停止上线；不得靠减少审计、增加预算、切换供应商或追加调用补救。
- 至少 80% 样本同时通过当条适用项目。
