...
- **选材改革扰动（2026-07-20 定）**：代码已实施但尚未部署；从部署后的首个有效产出日起重新计数，`removed_fields / audited_events` 安全基线改用**变更后前 3 个产出日的中位数**，验收窗口须覆盖至少 5 个变更后产出日。
...
- 选材改革会改变 `selected_before_audit` 样本构成，因此 7-day 线上指标门从改革部署后的首个有效 shadow 产出重新计数；45-case / three-run 固定夹具门不受影响。
...
观测字段已在选材改革中补齐：`source_health.json` 逐源记录抓取条目、参与评分事件和参与最终精选事件；shadow summary 记录高风险单源率、独立链和来源集中度。14 天窗口从这些字段部署后的首个有效产出日起计，不使用旧数据反推。

## #7 第一轮单变量调参（已取代）

2026-07-20 已被 `docs/news_selection_plan.md` 取代：其要做的测量由 07-10~19 十日数据提前完成且结论更强（8/10 天截断过线条目、被截条目分数高至 85，远超原"2/3 值得进"判定门）。选材改革（pick_max→32、保留席→3、统一质量线、按日等权动态百分位）已经实现，待部署后按该计划进入 7 个产出日验收。
