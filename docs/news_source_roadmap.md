...
本轮尚未宣称已部署或线上验收通过。获得公开启用授权后，选材改革与可信轨迹同日公开：选材独立连续计 7 个有效产出日，轨迹同时独立连续计 5 日，不以选材验收作轨迹前置。#15 是两门唯一的自动每日台账：选材用确定性指标初验，轨迹用 `audit_llm` 独立 Judge 初验，异常或不确定结果记 `needs_review` 并留给人工最终确认。
...
- **选材改革扰动（2026-07-20 定）**：代码已实施但尚未公开验收；从选材/轨迹并行公开后的首个有效产出日重新计数，但仍是独立的 enrich 五日门。`removed_fields / audited_events` 安全基线改用**新窗口前 3 个产出日的中位数**，验收窗口须覆盖至少 5 个变更后产出日。
...
2026-07-20 已被 `docs/news_selection_plan.md` 取代：其要做的测量由 07-10~19 十日数据提前完成且结论更强（8/10 天截断过线条目、被截条目分数高至 85，远超原"2/3 值得进"判定门）。选材改革（pick_max→32、保留席→3、统一质量线、按日等权动态百分位）已经实现，待获授权与可信轨迹同日公开后进入自己的 7 个产出日验收。
