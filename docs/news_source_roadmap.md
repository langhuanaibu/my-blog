...
活动 provider 已从 StepFun 切回 DeepSeek：StepFun 实验 Run #30346999214 在正常新闻阶段 A 触发 HTTP 451，不能作为生产回退；DeepSeek 的 45 条夹具三轮门（Run #30349424143）和 validate（Run #30350219791）通过后，2026-07-28 的首次有效 publish（Run #30353163162）成功。成本护栏随后改变了同日事件候选样本并进入共享运行时指纹，**下一次有效 publish 会再次重置全部五门**；新窗口完成前不得沿用当前连续天数，也不得按旧指纹推算收口日期。

**2026-07-28 可信台账快照**：选材 1/7、轨迹 1/5、enrich 0/5、客观性 shadow 1/7、信源指标 1/14；五门整体均未完成。该快照属于成本护栏生效前的运行时指纹，只用于说明当前状态；下一次有效 publish 后的状态与计数以 GitHub Issue #15 为准。
