...
2026-07-24 曾是温和扩充基线的首个有效产出日，07-27 又因 `build_enrich_sample` 修复重新起算。活动 provider 随后从 StepFun 切回 DeepSeek：StepFun 实验 Run #30346999214 在正常新闻阶段 A 触发 HTTP 451，不能作为生产回退；DeepSeek 的 45 条夹具三轮门（Run #30349424143）和 validate（Run #30350219791）通过后，2026-07-28 的首次有效 publish（Run #30353163162）成功。该次运行产生新的共享运行时指纹，**当前五门统一起点是 2026-07-28**，不得沿用此前连续天数，也不再按旧指纹推算收口日期。

**2026-07-28 台账快照**：选材 1/7、轨迹 1/5、enrich 0/5、客观性 shadow 1/7、信源指标 1/14；五门整体均未完成。当天除 enrich 外四门通过；enrich 的 `removed_fields / audited_events` 为 2.9697，高于固定 limit 1.9714，确定性失败。人工复核只能覆盖 `needs_review`，不能把机械指标的 `fail` 改成通过。后续状态与计数以 GitHub Issue #15 为准，本文件不维护逐日流水。
...
- 至少 80% 样本同时通过当条适用项目。
- 安全指标按 `removed_fields / audited_events` 计算：以窗口起点前最近 3 个产出日的中位数为基线，观察期不得超过基线的 1.2 倍；当前固定 limit 为 1.9714。2026-07-28 实测 2.9697，判定 `fail`。
...
- 选材改革或共享运行时指纹变化会改变 `selected_before_audit` 样本构成，因此 7-day 线上指标门从 2026-07-28 的首个有效 DeepSeek shadow 产出重新计数；它不被选材 7 日或轨迹 5 日代替，45-case / three-run 固定夹具门已按当前 provider 单独重跑。
...
观测字段已在选材改革中补齐：`source_health.json` 逐源记录抓取条目、参与评分事件和参与最终精选事件；shadow summary 记录高风险单源率、独立链和来源集中度。14 天窗口从 2026-07-28 的首个有效 DeepSeek 产出日起计，不使用旧数据反推，也不被两个上线门替代。

## #7 第一轮单变量调参（已取代）

2026-07-20 的单变量调参已由动态阈值与类目保留席改革取代：07-10~19 十日数据已有 8/10 天截断过线条目，被截条目分数高至 85，远超原“2/3 值得进”判定门。原选材改革先落地 32 条/3 席；温和扩充调整为 36 条/4 席，统一质量线、按日等权动态百分位、`pick_min: 8` 和 8 条「更多资讯」不变。现行机制与回退参数写在 `readme.md`，线上观察按上文当前生产基线计数。
