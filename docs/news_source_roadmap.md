...
**2026-07-29 追加**：删除字段分项诊断（`removed_field_counts` / `removed_field_reasons`）与详情页展示层改造在同一批合入，运行时指纹与轨迹 UI 指纹同时变化。因为它赶在修复后的首个有效 publish **之前**合并，五门此时一天都没起算，所以这批不额外损失任何连续天数——起算点仍是它合并后的首个有效 publish。分项数据积累 3-5 个有效日后才用于判断「空块是闸门删的还是生成端没写」，在那之前不得据此调整闸门或 enrich 提示词。
...
- `enabled: false`，待项目锁定的标准 CA 包能完成验证后复测再启用，届时按 **7 个有效日**（原 14 日）观察。
- 不占关键路径：队列其余部分不等它。
