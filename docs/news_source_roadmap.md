...
本次温和扩充把精选上限调为 36、五类保留席调为 4、深读上限调为 4，并加入内容类型、分离时长和漏读复盘。它会改变公开样本构成，因此发布后的首个有效产出日是新的统一基线起点；此前未完成的 selection、trajectory、enrich、objectivity shadow 和主管线 source metrics 均按各自有效日条件重新计时，不沿用旧连续天数。
