...
- 页面视觉与博客羊皮纸主题统一，亮/暗跟随博客：初始化读 Fluid 写入的 `localStorage["Fluid_Color_Scheme"]` 决定亮暗，未设置则跟随系统。阅读型日报、周报、时间线和详情页统一使用 780px 阅读栏，普通卡片采用轻阴影与明确边框，浮层保留独立阴影；五类新闻使用浅底分类色，状态标签继续使用事实状态色。耦合点：博客若更换主题或改这个存储键，日报页暗色会静默失效。
