...
- `daily/YYYY-MM-DD.js`：每日页面数据。`themes` 为"今日主线"（2-3 条，每条含 `member_ids` 引用当日 `pick-N`/`more-N` 条目，可跨精选与更多资讯）；每条精选还带 `context`（背景与机制）、`significance`（对我的意义，结合兴趣画像生成）和 `detail`（中文长叙述，约 300-600 字，供预览页讲清"究竟发生了什么"，只对精选 pick 生成，由 `config.yaml` 的 `detail` 段控制、关掉即不产出）。旧数据无这些字段时前端自动跳过/降级。
- `manifest.js`：日报日期清单。
...
- 精选卡的标题可点，或点卡片底部「读全文 →」进入**中文预览页**：页内路由，URL 形如 `/news/?date=YYYY-MM-DD&item=pick-N`，可分享、可回退（浏览器前进/后退与页内「返回当日」均走 `popstate` 路由），直接复用已 load 的 `window.NEWS_DATA`、不发新请求。正文取管线生成的 `detail` 长叙述并按空行分段，另附短字段小节、跨天进展、来源链接。历史日报的精选无 `detail`，点开时优雅降级为短字段。
