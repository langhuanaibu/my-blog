...
- `all/YYYY-MM-DD.js` + `all/manifest.js`：全量轻档——抓取窗口内全部条目的轻字段（标题/链接/来源/类别/时间），不经 LLM、零 token，管线抓取去重后立即写入（`write_all_archive`），滚动保留 90 天。前端「全部动态」Tab 按天懒加载，作为筛选器的可核查底账；写失败只记日志、不阻断主管线。

### 页面能力

- 顶部主导航为四 Tab（AIHOT 式板块划分）：**精选**（跨天连续流，最新在上、按天分组、每批 5 天分批加载，复用精选卡与反馈操作；跨天条目用 `日期:id` 复合引用键，反馈/稍后读按条目所属日期记账，避免日内 `pick-N` 编号跨天碰撞）、**全部动态**（全量轻档流，未经筛选、保留 90 天）、**日报**（默认落地 Tab，日期控件与日/周切换仅在此显示）、**主题**。搜索全局可用；稍后读/单词本入口（仅 token）不随 Tab 隐藏。
