...
- 精选采用阈值制，不硬凑固定条数；同时保留类目保底、上限、同源封顶和受控主题标签。另有单类精选上限（`config.yaml` 的 `max_per_category`，现仅 `ai: 3`）：日报 AI 板块定位为"头条提醒器"（AI 深度阅读直接看 AIHOT 网站），超限高分条目降级进"更多资讯"不丢失。主题标签只允许来自 `config.yaml` 的 `topic_tags`，词表外标签会被丢弃。
...
- `daily/YYYY-MM-DD.js`：每日页面数据。`themes` 为"今日主线"（2-3 条，每条含 `member_ids` 引用当日 `pick-N`/`more-N` 条目，可跨精选与更多资讯）；每条精选还带 `context`（背景与机制）、`significance`（对我的意义，结合兴趣画像生成）、`detail`（中文长叙述，约 300-600 字，由 `config.yaml` 的 `detail` 段控制）和 `claims`（2-4 条关键结论，`{text, kind: fact|analysis|uncertain, sources: [来源名]}`——管线在 `sanitize_claims` 阶段就把 LLM 的来源索引解析成来源名，因为 `build_item` 会对来源重排去重，索引跨层无稳定含义）。深读条目带 `key_points`（≤3 条）/`audience`/`takeaway`，论文条目带 `contribution`/`evidence`/`limitations`/`takeaway`，供详情页渲染。旧数据无这些字段时前端自动跳过/降级（详情入口也随之退回原文外链）。
- `manifest.js`：日报日期清单。
...
- 顶部主导航为四 Tab（AIHOT 式板块划分）：**精选**、**全部动态**、**日报**（默认落地 Tab，日期控件与日/周切换仅在此显示）、**主题**；持 token 时追加第五个 **⭐ 收藏** Tab（访客不可见，URL `?view=favs` 无 token 时回落日报）。搜索全局可用；稍后读入口（仅 token）不随 Tab 隐藏。跨天条目统一用 `日期:id` 复合引用键，反馈/稍后读按条目所属日期记账，避免日内 `pick-N` 编号跨天碰撞。
...
- 精选 Tab：单列时间轴（左侧竖线 + 圆点 + 精确 HH:MM），条目按原文发布时间转换为北京时间后归日，日报文件日期只代表生成批次；无有效发布时间的旧条目回退日报日期并在时间后标 `?`。同一跨天事件的纯重复报道合并来源，有实质新增时保留节点并标「持续事件」。顶部「🔥 当前热点」将近 3 个日报批次按稳定 `event_id` 聚合（旧数据以规范化 URL/保守标题相似度兜底），同一事件只占一席；独立信源按去重域名计数，再乘 36 小时时间衰减排 Top 6。页内检索与普通时间轴采用相同归日和去重口径。
...
- **统一详情页**（定位是"决策/分析页"而非 AIHOT 式的原文阅读页——原文一键出站）：页内路由 `/news/?date=YYYY-MM-DD&type=news|deep|paper&item=<id>`（旧式无 `type` 的链接默认 news，仍兼容），可分享、可回退，复用已 load 的 `window.NEWS_DATA` 不发新请求。三类内容各有版式：新闻＝takeaway 大字卡（取 significance，缺省 why；两者都有时 why 回到字段区不丢失）→ 摘要 → detail 长叙述 →「结论与证据」claims 三色卡（事实/判断/待核实，附来源名）→ 背景/后续关注 → 事件进展 → 来源（移动端超 2 条折叠）；深读＝读完带走 + 核心观点 + 适合谁读 + 打开原文；论文＝学习价值 + 核心贡献/证据/适用边界 + 打开原文。详情页通用：绝对+相对双时间、「⧉ 导出 MD」（复制 Markdown，剪贴板不可用时降级下载）、持 token 时底部有稍后读/收藏等操作行。**入口规则是"有料才进详情"**：精选标题/「查看详情 →」恒进详情；深读/论文标题仅当条目带新详情字段（takeaway/key_points 等，新管线起产出）时进详情，历史条目仍直达原文外链；「更多资讯」行恒为外链。历史精选无 `detail`/claims 时详情页按已有短字段优雅降级。
