...
- 模型返回的形状一律不可信：任何 `json_call` 调用点都必须自己校验，形状非法时降级而不是中止整跑，且降级方向必须不丢内容。阶段A 曾因缺这层防护，被一批「只返回单个对象」的回复整跑打挂（2026-08-01，Run #30674149780，当天无日报）。现在整批不可用时该批每条降级为单条事件，由全量同日归并接住重复；非法元素与整批降级分别记 `triage_invalid_rows` / `triage_fallback_batches`，只做诊断、不参与验收门。新增调用点照此办理，理由与被否掉的方案见 `docs/adr/0012-model-shape-failures-degrade-not-abort.md`。
