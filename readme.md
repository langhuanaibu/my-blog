...
- 确需人工裁决 `needs_review` 时，使用只读证据 artifact 复核后手动触发 `Rollout Manual Review`。该工作流只能由 Actions bot 修改同一日期的可信台账，只接受与最新 run 一致的 `pass` / `fail` / `neutral`，不能覆盖发布失败或自动确定性结论；后续新 run 会自动使旧人工结论失效。
