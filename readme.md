...
- 顶部主导航为四 Tab（AIHOT 式板块划分）：**精选**、**全部动态**、**日报**（默认落地 Tab，日期控件与日/周切换仅在此显示）、**主题**。搜索全局可用；稍后读/单词本入口（仅 token）不随 Tab 隐藏。跨天条目统一用 `日期:id` 复合引用键，反馈/稍后读按条目所属日期记账，避免日内 `pick-N` 编号跨天碰撞。
...
- 精选卡的标题可点，或点卡片底部「读全文 →」进入**中文预览页**：页内路由，URL 形如 `/news/?date=YYYY-MM-DD&item=pick-N`，可分享、可回退（浏览器前进/后退与页内「返回当日」均走 `popstate` 路由），直接复用已 load 的 `window.NEWS_DATA`、不发新请求。正文取管线生成的 `detail` 长叙述并按空行分段，另附短字段小节、跨天进展、来源链接、及当日归属本条的相关单词（`item_id` 命中）。历史日报的精选无 `detail`，点开时优雅降级为短字段。
