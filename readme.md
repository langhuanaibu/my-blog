...
- 客观性模型验收：`python news-pipeline/objectivity_eval.py`。语料固定为仓库内 45 条夹具（`news-pipeline/fixtures/`，受 canonical JSON 的 SHA-256 约束，不能用 CLI 换），每条先走生产 `enrich` 与完整 audit/repair/fallback，再交独立 judge 评分；生成模型和 judge 都拿不到单条夹具的分类、预期标签和验收阈值。Judge 使用固定的全局风险词表：标签只分类证据中的风险场景，`redlines` 只报告最终候选仍存在的违规，危险主张已省略时不判归因失败。只有显式配置了线上 LLM 凭证才会连跑三轮，并按最差一轮决定退出码；聚合报告分列非法案例、类别不匹配、Judge 报告红线与归因分子/分母，便于区分模型输出故障和内容问题。Actions 的 `Objectivity Acceptance` 可手动触发，复用仓库 Secret、只读运行并上传保留 14 天的聚合报告，不提交任何数据；自动化测试只用本地桩，不调付费接口。
...
- Judge 返回缺行、重复行、越界编号或非法字段时，轨迹初验会以同一输入重试一次；客观性固定夹具会把非法批次递归拆小，单条仍非法时再重试一次。重试只修复输出结构，不改变证据、标签、红线或验收阈值。
- 确需人工裁决 `needs_review` 时，使用只读证据 artifact 复核后手动触发 `Rollout Manual Review`。该工作流只能由 Actions bot 修改同一日期的可信台账，只接受与最新 run ID 和 attempt 一致的 `pass` / `fail` / `neutral`，不能覆盖发布失败或自动确定性结论；后续新 run 或 rerun 会自动使旧人工结论失效。
