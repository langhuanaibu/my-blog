...
- GitHub Actions（`.github/workflows/daily-news.yml`）每天 UTC 23:00（北京 07:00 左右）运行管线，校验通过后自动 commit + push `source/news/data/`，触发 Vercel 部署上线。这是"严禁自动 push"规则的唯一例外，详见 `CLAUDE.md`。同一 workflow 在 `generate` 与只读 `shadow` 结束后运行云端 `rollout-review`：读取临时验收 artifact，使用最小权限 `contents: read` / `issues: write` 幂等更新 GitHub Issue #15；Issue 关闭后自动跳过。下载后的 shadow 状态固定读取 `shadow-status/daily-news-shadow-status.json`，缺失、损坏或非 `success` 一律保守判失败。完整 `rollout-report.json` 另存为保留 14 天的 Actions artifact，供人工复核，不写入公开数据或 Issue 正文。该台账任务完全运行在 GitHub 托管 runner，不依赖个人电脑或 Codex Desktop 在线。
...
- 客观性模型验收：`python news-pipeline/objectivity_eval.py`。语料固定为仓库内 45 条夹具（`news-pipeline/fixtures/`，受 canonical JSON 的 SHA-256 约束，不能用 CLI 换），每条先走生产 `enrich` 与完整 audit/repair/fallback，再交独立 judge 评分；生成模型和 judge 都拿不到单条夹具的分类、预期标签和验收阈值。Judge 使用固定的全局风险词表：标签分类证据**为写作者制造**的风险（含「原文干净但可能被添加」的缺席型风险，不得回答无风险），`redlines` 只报告最终候选仍存在的违规，危险主张已省略时不判归因失败。夹具用 `expected.accepted_labels` 列出所有据文站得住的标签以容纳类别接缝，Judge 仍只选一个。只有显式配置了线上 LLM 凭证才会连跑三轮，并按最差一轮决定退出码；非法批次会递归拆小，单条重试后仍非法则保守记为结构失败，每轮最多调用 Judge 60 次。聚合报告分列非法案例、类别不匹配、Judge 报告红线与归因分子/分母，并附逐条 `case_findings`（失败条目 id、接受集与 Judge 实际作答），便于区分模型输出故障和内容问题。Actions 的 `Objectivity Acceptance` 仅允许从 `main` 手动触发，复用仓库 Secret、只读运行并上传保留 14 天的聚合报告，不提交任何数据；自动化测试只用本地桩，不调付费接口。
