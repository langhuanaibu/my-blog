...
- 客观性模型验收：`python news-pipeline/objectivity_eval.py`。语料固定为仓库内 45 条夹具（`news-pipeline/fixtures/`，受 canonical JSON 的 SHA-256 约束，不能用 CLI 换），每条先走生产 `enrich` 与完整 audit/repair/fallback，再交独立 judge 评分；生成模型和 judge 都拿不到夹具分类、预期标签和验收阈值。只有显式配置了线上 LLM 凭证才会连跑三轮，并按最差一轮决定退出码；聚合报告分列非法案例、类别不匹配、Judge 报告红线与归因分子/分母，便于区分模型输出故障和内容问题。Actions 的 `Objectivity Acceptance` 可手动触发，复用仓库 Secret、只读运行并上传保留 14 天的聚合报告，不提交任何数据；自动化测试只用本地桩，不调付费接口。
