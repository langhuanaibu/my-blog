...
- 数据推上 `main` 不等于线上能读到。2026-08-01 机器人提交 `2623d4c` 后 Vercel 没有收到 webhook，日报在 `main` 上完好却一整天没上线，且完全静默（`vercel.json` 无 `ignoreCommand`、`.vercelignore` 只排除个人状态文件，已排除仓库侧原因）。`daily-news.yml` 新增独立 job `deploy-check`：只在确实提交了数据时运行，带 cache-bust 轮询线上 `manifest.js` 最长 10 分钟；未上线则调用 Secret `VERCEL_DEPLOY_HOOK` 自愈并再等一轮，仍失败就让该 job 显红。**该 Secret 必须是 Vercel 项目 `my-blog` 的 Deploy Hook**：生产站点 `aoiblog.top` 与 `api.aoiblog.top` 都由 `my-blog` 提供服务，仓库另接了一个 `my-blog-ynww`，它不承担任何生产域名、只是跟着每次提交一起构建。钩子建错项目时自愈会去部署不服务主站的那个，站点照样不更新，而日志里看不出任何差别。这条归属关系在代码里查不到——`vercel.json` 是共享的，前端调 API 走同源 `fetch('/api/...')`，只能到 Vercel 控制台的 Settings → Domains 确认。**该 job 刻意不进 `rollout-review` 的 `needs`**——`publication` 取自 `generate` 的结果，把部署校验塞进 `generate` 会让 Vercel 打个嗝就把当天判成发布失败、五门归零。Secret 未配置时自动降级为只检测告警。注意 Vercel 界面上对旧 commit 点 Redeploy 解决不了这类问题，它重建的是那个 commit 的树。
