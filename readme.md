...
8. **全量支持 Markdown 沉浸式写作与渲染**：
   - 彻底解决了只能在后台写 HTML 的痛点。在 `admin.html` 引入“导入MD”功能，支持一键上传本地 `.md` 文件，智能提取一级标题并填充。
   - 前端 (`articles.html` & `index.html`) 接入 `marked.js` 实时渲染 Markdown 内容，完美兼容现有的 Prism.js 代码高亮和动态 TOC 大纲提取，且保持对历史 HTML 文章的 100% 向后兼容。
