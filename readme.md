...
- 客观性模型验收：`python news-pipeline/objectivity_eval.py`。语料固定为仓库内 45 条夹具（`news-pipeline/fixtures/`，受 canonical JSON 的 SHA-256 约束，不能用 CLI 换），每条先走生产 `enrich` 与完整 audit/repair/fallback，再交独立 judge 评分；生成模型和 judge 都拿不到单条夹具的分类、预期标签和验收阈值。Judge 使用固定的全局风险词表：标签只分类证据中的风险场景，`redlines` 只报告最终候选仍存在的违规，危险主张已省略时不判归因失败。只有显式配置了线上 LLM 凭证才会连跑三轮，并按最差一轮决定退出码；非法批次会递归拆小，单条重试后仍非法则保守记为结构失败，每轮最多调用 Judge 60 次。聚合报告分列非法案例、类别不匹配、Judge 报告红线与归因分子/分母，便于区分模型输出故障和内容问题。Actions 的 `Objectivity Acceptance` 仅允许从 `main` 手动触发，复用仓库 Secret、只读运行并上传保留 14 天的聚合报告，不提交任何数据；自动化测试只用本地桩，不调付费接口。
...
- 45 条客观性夹具仍由独立的手动 `Objectivity Acceptance` workflow 执行（一次 dispatch 内跑满三轮，取三轮最差值判 红线 0 / 归因 ≥95% / 结构 100%），不属于每日自动台账。
